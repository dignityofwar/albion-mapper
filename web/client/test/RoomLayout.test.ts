...
describe('RoomView node positioning', () => {
  it('should place new nodes to the RIGHT of the SOURCE node', async () => {
    ...
    // Position of 'new-node' should be 'home'.x + 350 = 350
    const newNode = vm.flowNodes.find((n: any) => n.id === 'new-node');
    expect(newNode).toBeDefined();
    expect(newNode.position.x).toBe(350);
    expect(newNode.position.y).toBe(0);
    
    wrapper.unmount();
  });

  it('should place new nodes to the RIGHT of the SOURCE node when source is at X 350', async () => {
    setActivePinia(createPinia());
    const store = useRoomStore();

    // Setup initial state: Home node at (0,0), Source node at (350, 0)
    store.applyMessage({ 
        type: 'sync', 
        connections: [], 
        homeZoneId: 'home',
    ...
    // Position of 'new-node' should be 'source'.x + 350 = 700
    const newNode = vm.flowNodes.find((n: any) => n.id === 'new-node');
    expect(newNode).toBeDefined();
  ...
  it('should place new nodes to the LEFT of the SOURCE node when source is at X 500 but Home is at X 1000', async () => {
    ...
    // Position of 'new-node' should be 'source'.x - 350 = 150
